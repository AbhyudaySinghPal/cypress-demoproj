...
describe("DemoQA API testing demo", () => {
  it("Create User Emulation", function () {
    const userId: string = "7d01de84-9527-4855-a10c-043a637178b3";
    cy.wrap(userId).as("userId");
  });

  it("Get Auth token from login", function () {
  ...
  it("get User info", function () {
    ...
    cy.request(options).then((response) => {
      ...
      cy.log("Book title").then(() => {
        expect(response.body.books[0].title).to.be.equal(
